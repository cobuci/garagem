...
<x-modal-card wire:model="showConfirmPaymentModal" title="{{ __('sales.confirm_payment') }}" max-width="md">
        ...
        @if($selectedSale)
            <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 space-y-2 border border-gray-100 dark:border-gray-700">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-bold text-primary-600">R$ {{ number_format($selectedSale->total_amount, 2, ',', '.') }}</span>
                </div>
            </div>
        @endif
    </div>
    ...
    <x-slot name="footer">
        <div class="flex justify-end gap-x-4">
            ...
            <x-button primary label="{{ __('sales.yes_mark_as_paid') }}" wire:click="markAsPaid" spinner="markAsPaid" />
        </div>
    </x-slot>
</x-modal-card>
