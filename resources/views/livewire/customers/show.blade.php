<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <div class="flex space-x-3">
            <button wire:click="$dispatch('edit:customer', { customer: {{ $customer->id }} }   )" class="bg-white border border-gray-200 text-gray-700 px-4 py-2 rounded-lg font-medium hover:bg-gray-50 transition shadow-sm">
                {{ __('customers.edit') }}
            </button>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white">R$ {{ number_format($this->totalSpent / 100, 2, ',', '.') }}</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            ...
            <p class="text-2xl font-bold text-red-600 dark:text-red-400">R$ {{ number_format($this->totalDue / 100, 2, ',', '.') }}</p>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900/50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('orders.paid') }}</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse ($this->orders as $order)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900 dark:text-white">R$ {{ number_format($order->total_amount / 100, 2, ',', '.') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-600 dark:text-gray-300">R$ {{ number_format($order->paid_amount / 100, 2, ',', '.') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    $statusClasses = [
                                ...
                                @endphp
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusClasses[$order->status] ?? 'bg-gray-100 text-gray-700 dark:bg-gray-900 dark:text-gray-400' }}">
                                    {{ $statusLabels[$order->status] ?? $order->status }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <button class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">{{ __('orders.view_details') }}</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
