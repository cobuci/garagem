    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="relative overflow-hidden bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 group hover:shadow-md transition-shadow duration-300">
            ...
            <div class="relative">
                ...
                <div class="mt-2 flex items-baseline">
                    <p class="text-2xl font-bold text-gray-900 dark:text-white leading-none">R$ {{ number_format($this->totalBalance, 2, ',', '.') }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="relative">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-right">
                        <span class="text-xs font-bold text-amber-600 dark:text-amber-400">{{ number_format($this->goalMetrics['percent'], 1) }}%</span>
                    </div>
                </div>
                ...
                <div class="mt-2" x-data="{ editing: false }">
                    <div x-show="!editing" @click="editing = true" class="cursor-pointer group/goal">
                        <p class="text-2xl font-bold text-gray-900 dark:text-white leading-none flex items-center gap-2">
                            R$ {{ number_format($this->targetBalance, 2, ',', '.') }}
                    ...
                    <div class="mt-3">
                        <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2 mb-1">
                            <div
                                ...
                                style="width: {{ $this->goalMetrics['percent'] }}%"
                            ></div>
                        </div>
                        <p class="text-[10px] font-bold uppercase tracking-tight {{ $this->goalMetrics['reached'] ? 'text-emerald-600' : 'text-gray-400' }}">
                            @if($this->goalMetrics['reached'])
                                {{ __('dashboard.goal_reached') }}
                            @else
                                {{ __('dashboard.remaining_to_goal', ['amount' => 'R$ ' . number_format($this->goalMetrics['remaining'], 2, ',', '.')]) }}
                            @endif
                        </p>
                    </div>
                </div>
        ...
        <div class="relative overflow-hidden bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 group hover:shadow-md transition-shadow duration-300">
            ...
            <div class="relative">
                ...
                <div class="mt-2">
                    <p class="text-2xl font-bold text-gray-900 dark:text-white leading-none">R$ {{ number_format($this->dailyMetrics['sales'], 2, ',', '.') }}</p>
        ...
        <div class="relative overflow-hidden bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 group hover:shadow-md transition-shadow duration-300">
            ...
            <div class="relative">
                ...
                <div class="mt-2">
                    <p class="text-2xl font-bold text-gray-900 dark:text-white leading-none">R$ {{ number_format($this->monthlyMetrics['sales'], 2, ',', '.') }}</p>
                    <div class="mt-3 grid grid-cols-2 gap-2 text-sm">
                        <div class="flex flex-col">
                            <span class="text-gray-400 text-[10px] uppercase font-bold">{{ __('dashboard.profit') }}</span>
